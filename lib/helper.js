...
helper.clone_to_dir = function(push, callback) {
	push.cwd = path.normalize(push.cwd);
	push.repoNname = path.basename(push.cwd, '.git');
	var command = "git clone file://"+push.cwd+" --depth=1 --branch "+push.branch+" "+push.repoName+"/"+push.commit+"/";
